...
The magnitude sits inside typical crystallographic refinement uncertainty for BM5.5-range resolutions. BM5.5 spans ~1.4-3.5 Å resolution with median around 2.2 Å. At those resolutions, the diffraction-data-precision index (DPI; Cruickshank 1999) and Luzzati / Sigma-A coordinate-error estimates run 0.1-0.3 Å for whole-structure DPI, with interface side chains often above the bulk DPI due to elevated B-factors. Independent force-field-vs-crystal benchmarking literature reports comparable magnitudes for restrained AMBER minimization on protein crystal coordinates: Lindorff-Larsen et al. (2012, *PLoS ONE*) measured force-field-induced backbone drift of 0.5-1.5 Å under unrestrained MD; Raval et al. (2012, *Proteins*) reported 0.2-0.4 Å under restrained minimization for homology-model refinement; Beauchamp et al. (2012, *J. Chem. Theory Comput.*) benchmarked force-field-vs-NMR concordance with similar coordinate-uncertainty findings. AMBER moves the BM5.5 crystal coordinates by an amount comparable in magnitude to the underlying refinement uncertainty, but in a systematic direction the refinement does not have.

**Practical reading**: AMBER applied to predictions improves them; AMBER applied to crystals produces a small, systematic perturbation comparable to the refinement-uncertainty envelope. For comparisons where both prediction and crystal sources are AMBER-treated, the comparison is fair; for analyses that retain the crystal as an unmodified reference, skip AMBER on the crystal source.

**References for the AMBER-on-crystal magnitude claim**:
- Cruickshank, D.W.J. Remarks about protein structure precision. *Acta Crystallogr. D Biol. Crystallogr.* 55, 583-601 (1999).
- Lindorff-Larsen, K., Maragakis, P., Piana, S., Eastwood, M.P., Dror, R.O., Shaw, D.E. Systematic validation of protein force fields against experimental data. *PLoS ONE* 7(2): e32131 (2012).
