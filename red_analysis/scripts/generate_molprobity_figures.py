...
def fig12_molprobity_by_source(data):
    """Grouped bar charts for key MolProbity metrics. Both pipelines."""
    ...
    for pipeline in ['blue', 'green']:
        ...
        per_target = data[data['pipeline'] == pipeline].groupby(['source', 'target'])[
        ...
        fig.suptitle(f'MolProbity Validation Metrics by Structure Source\n({pipe_label} pipeline, per-target averages)',
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_fig(fig, f'fig12_molprobity_by_source_{pipeline}')
...
def fig14_tm_vs_molprobity(data):
    """THE KEY FIGURE: TM-score (no change) vs MolProbity (big change). Both pipelines."""
    ...
    for pipe in ['blue', 'green']:
        ...
        fig.suptitle(f'AMBER Dual Effect: Zero TM Change, Large MolProbity Improvement\n({pipe_label} pipeline)',
                    fontsize=14, fontweight='bold')
        plt.tight_layout()
        save_fig(fig, f'fig14_amber_dual_effect_{pipe}')


def fig15_crystal_vs_predicted(data):
    """Crystal structures have worse MolProbity than predictions. Both pipelines."""
    for pipe in ['blue', 'green']:
        pipe_label = pipe.capitalize()
        ...
        ax.set_title(f'MolProbity: Crystal vs Predicted ({pipe_label})\n(Crystal has worst rotamers and many clashes)',
        ...
        plt.tight_layout()
        save_fig(fig, f'fig15_crystal_vs_predicted_{pipe}')
