...
SCATTER_BOUNDS = {
    ...
    'rota_outliers':    (-0.5,  18,     -0.5,   0.5),   # x p99=16.7; errbar top max=0.47; negative y axis
...
def make_per_target_bars(mp, ros, metric, label, lower_better):
    ...
    for row_idx, row_cfg in enumerate(BAR_ROWS):
        ...
        # Y-axis: show full range (initial bars NOT clipped) with negative
        # padding so error bars at y=0 are fully visible.
        ...
        if init_vals:
            ...
            ypad = ymax * 0.03  # 3% negative padding
            ax.set_ylim(-ypad, max(ymax, 0.1))
        ...
        if row_idx == 0:
            ...
            ax.legend(handles=handles, loc='upper left',
                      bbox_to_anchor=(0.0, 1.35), fontsize=6,
                      framealpha=0.9, ncol=7)
