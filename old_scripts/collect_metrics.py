...
def _parse_rosetta_total_from_pdbstr(pdbstr:str):
    if not pdbstr:
        return None
    # pattern A: a REMARK/SCORE line that literally contains "total_score <number>"
    m = re.search(r'(?:^|\n)REMARK.*?(?:SCORE|score).*?total_score\s+([+-]?\d+(?:\.\d+)?)', pdbstr)
    if m:
        try: return float(m.group(1))
        except: pass
    # pattern B: tabular "REMARK  score ... total_score" header followed by numeric row
    ...
    for i, txt in enumerate(lines):
        ...
        if 'total_score' in toks and ('score' in toks or 'fa_atr' in toks):
            hdr = toks
            # find next REMARK line that looks like all numbers
            for j in range(i+1, len(lines)):
                dtoks = lines[j].split()
                if not dtoks:
                    continue
                # try to read same-column value
                try:
    ...
    with open(path) as fh:
        for ln in fh:
            if not ln.startswith('SCORE:'):
                continue
            toks = ln.split()
            # header line has column names (includes 'description' usually)
...
def collect_metrics(ref='crystal', sel='polymer and name CA', do_fit=1, scorefile=None, out='metrics_to_crystal.tsv'):
    ...
    for obj in objs:
        ...
        # Energy (Rosetta total_score if available)
        energy = scoremap.get(obj, None)
        if energy is None:
            pdbstr = cmd.get_pdbstr(obj)  # may not contain original REMARKs, but try
    ...
    def _rkey(r):  # sort putting NaNs last
