...
**1. AMBER fixes prediction defects without meaningfully changing global fold or binding interface.** AMBER reduces clashscore with Cliff's d = -0.99 across all paired comparisons on prediction sources (mean drop 13-21 clashes per target, p < 1e-17). AMBER improves MolProbity for 257 of 257 AlphaFold targets and 256 of 257 Boltz targets (1Y64 is the one Boltz outlier where AMBER raises MolProbity by 0.46). TM-score mean shift is < 0.001 absolute (Cliff's d -0.6 to -0.8 reflects rank-ordered consistency, not biological magnitude); DockQ mean shift < 0.0002, well below DockQ's practitioner decision threshold of ~0.05.

**2. AMBER on crystal references produces a small structural perturbation comparable to crystallographic refinement uncertainty.** Median Δ i-RMSD 0.246 Å, Δ DockQ -0.022; the shift is directionally consistent on 257/257 targets but in magnitude comparable to Luzzati / Sigma-A coordinate uncertainty (0.2-0.3 Å typical at the BM5.5 ~2.2 Å median resolution). Cliff's d = -1.0 and p approach zero by construction (crystal vs crystal is DockQ 1.0 by definition; AMBER can only move it down). Practical implication: skip AMBER on crystal sources held as unmodified references; apply it symmetrically when both prediction and crystal sources are AMBER-treated.

**3. Post-Rosetta paired AMBER effect is small, directionally consistent, and below the practitioner noise floor.** Zero of 16 cells reach paired-t p < 0.05 at the locked frame; two reach Wilcoxon p < 0.05 (Blue amber_boltz vs boltz on dualspace_beta, mean Δ MP -0.005, Δ clashscore -0.02). Direction of all marginally-detected cells favors AMBER. Magnitude (~1-2% of cell mean) sits below the n = 257 paired-t minimum detectable effect. AMBER's role is **geometry pre-conditioner** (the Section 1 effect), not Rosetta-MolProbity scoring enhancer.

**4. `dualspace_beta` and `normal_beta` tie at the top of integrated MolProbity (within 0.01 MP units), and beta_nov16 dominates ref2015 systematically.** Best AMBER + Rosetta cell: `amber_boltz + dualspace_beta`, MP = 0.222. Best standalone Rosetta cell: `boltz + normal_beta`, MP = 0.213. Across all 42 (pipeline, source, move-set) triples, beta_nov16 beats ref2015 on MP score, clashscore, and Rama-favored (40-42 of 42); ref2015 wins on rotamer outliers (21 of 42). `cartesian_beta` is the TM-retentive runner-up; `ref2015` is the right default only when sidechain rotamer fidelity is the binding constraint.

**5. Crystal MolProbity is the highest in the comparison (clashscore 13.85 vs predictions 0.68-2.82), framed as score-function idealization.** Predictions plus AMBER plus Rosetta produce structures closer to the score-function ideal than the crystal references they were predicted against. The framing is idealization, not crystallographer failure: predictions come from neural networks trained on energy-minimized targets, AMBER perfects the physics in vacuo, Rosetta beta_nov16 adds another round of geometric idealization, while crystals reflect cryogenic packing and refinement constraints. Crystals remain canonical for global fold; predictions plus AMBER produce cleaner local geometry without losing it.
